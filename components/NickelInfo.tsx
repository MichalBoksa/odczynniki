...
const NickelInfo = ({ productLinks }: { productLinks: { cas: string; symbol: string; href: string }[] }) => {
  const { data } = useLocale() || {};

  return (
    <section className='max-container md:padding-container'>
      <h1 className='text-4xl md:text-6xl lg:text-8xl w-full mb-10 text-center'>{data?.NICKEL}</h1>
      ...
      <div className='flex flex-col lg:flex-row gap-12 mb-20 mt-20 items-center'>
        <div className="flex flex-col card w-72 h-48 mb-6 bg-base-100 border-primary border-2 items-center justify-center">
          <div className='flex flex-col items-center'>
            ...
            <p className='text-primary text-6xl font-bold'>Ni</p>
          </div>
        ...
        <div className='flex flex-2 mt-10 lg:mt-0'>
          <p className='px-3 lg:px-10 font-normal text-lg text-center lg:text-left'>{data?.NICKEL_DESC}</p>
        </div>
      </div>
      ...
      <div className='flex flex-col rounded border-solid border-2 border-base-200 py-4 mb-4'>
        ...
        <div className='flex flex-col gap-10 mt-6 pb-3'>
          {data?.NICKEL_PRODUCTS.map((product, index) => (
            <div key={product.symbol} className={`flex flex-col md:flex-row gap-7 items-center justify-center ${index !== data?.NICKEL_PRODUCTS.length - 1 ? 'border-b-2 border-b-gray-300 pb-8' : ''}`}>
