...
const SingleNewsPage: React.FC<NewsProps> = ({ posts, postsEng }) => {
  ...
  const firstPost = currentLocale === 'pl' ? (posts[0] ?? postsEng[0]) : (postsEng[0] ?? posts[0]);
    ...
    <section className='max-container md:padding-container lg:mb-32'>
      <div className='flex flex-col lg:flex-row gap-12 items-center justify-center'>
        ...
        <div className='flex-1 w-full mt-6 lg:mt-0'>
          ...
          <time dateTime={new Date(firstPost.createdAt).toISOString()} className="block mt-4">{new Date(firstPost.createdAt).toISOString().slice(0, 10)}</time>
        </div>
      </div>
